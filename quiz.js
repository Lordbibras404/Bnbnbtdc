...
function pickAnswer(idx) {
  ...
  /* Haptic + animation feedback on the tapped button */
  const tappedBtn = document.querySelectorAll('.quiz-option-btn')[idx];
  if (tappedBtn) {
    if (correct) {
      if (typeof hapticCorrect === 'function') hapticCorrect(tappedBtn);
    } else {
      if (typeof hapticWrong === 'function') hapticWrong(tappedBtn);
    }
  }
