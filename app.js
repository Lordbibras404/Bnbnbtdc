...
function initApp() {
  ...
  /* FIX: Telegram Mini App — window 'load' قد لا يُطلق
     نستخدم timeout مباشر بدل Promise.all لضمان التشغيل */
  const minDelay = 2800; // ms — وقت كافٍ للـ splash animation

  function _boot() {
    // نضمن تحميل كل المكونات قبل البدء
    if (typeof loadState !== 'function' || typeof QUESTIONS === 'undefined') {
      // لو المكونات لم تُحمّل بعد، ننتظر 200ms إضافية
  ...
  try {
    ...
    setTimeout(() => {
      try {
        ...
        console.error('[GRIT] bootApp inner error:', e);
        // Fallback: show onboarding to avoid blank screen
        try { showOnboarding(); } catch(_) {}
      }
    }, 420);
  } catch(e) {
    console.error('[GRIT] bootApp error:', e);
    // Hard fallback
...
function initTelegram() {
  ...
  // ضبط ارتفاع الـ viewport بشكل صحيح داخل Telegram
  ...
  try {
    ...
    if (_obStep === 2) {
      ...
      if (!name || name.length < 2) {
        if (inp) { inp.classList.add('shake'); setTimeout(function(){ inp.classList.remove('shake'); }, 500); }
        if (typeof showToast === 'function') showToast('أدخل اسمك أولاً ✏️', 'warning');
        return;
      }
      st.name = name;
      saveState(st);
    ...
    if (_obStep === 3) {
      ...
      if (!sel) {
        if (typeof showToast === 'function') showToast('اختر شعبتك أولاً 🎓', 'warning');
        return;
      }
    ...
    console.error('[GRIT] obNext error:', e);
    // Fallback: skip onboarding and go to app
    try {
      const st = loadState();
      st.onboarded = true;
      if (!st.name) st.name = 'طالب';
      if (!st.branch) st.branch = 'science';
      saveState(st);
      finishOnboarding();
    } catch(_) {}
  }
...
function finishOnboarding() {
  try {
    const ov = document.getElementById('onboardingOverlay');
    if (ov) {
      ov.style.animation = 'fadeOut 0.4s ease forwards';
      setTimeout(function() { ov.style.display = 'none'; }, 380);
    }
    showMainApp();
    renderHome();
    if (typeof checkStreak === 'function') checkStreak();
    if (typeof showToast === 'function') showToast('مرحباً بك في GRIT Learn 🎉', 'success');
  } catch(e) {
    console.error('[GRIT] finishOnboarding error:', e);
    // Hard fallback
    const ov = document.getElementById('onboardingOverlay');
...
function showMainApp() {
  ...
  initNavIndicator();
  _initHeroParallax();
  _wireQuickStartRipple();
}

/* Lightweight hero parallax — updates CSS vars on home content scroll.
   Uses one passive scroll listener. Max translation: 18px (orb A), 10px (orb B).
   Stops tracking when home screen is not active to save battery. */
function _initHeroParallax() {
  const homeContent = document.querySelector('#homeScreen .content');
  ...
  homeContent.addEventListener('scroll', function() {
    ...
    requestAnimationFrame(function() {
      const y = homeContent.scrollTop;
      const hero = document.querySelector('.home-hero');
      if (hero) {
        // Orb A moves faster (closer layer) — max 18px down
        hero.style.setProperty('--orb-a-y', Math.min(y * 0.18, 18) + 'px');
        // Orb B moves slower (further layer) — max 10px down, slight x drift
        hero.style.setProperty('--orb-b-y', Math.min(y * 0.10, 10) + 'px');
      }
      ticking = false;
    });
  }, { passive: true });
...
/* Wire ripple to the quick-start button (injected after DOM ready) */
function _wireQuickStartRipple() {
  const btn = document.querySelector('.quiz-quick-start-btn');
  if (!btn) return;
  btn.addEventListener('click', function(e) {
    _rippleOnElement(btn, e.clientX, e.clientY, 'white');
    hapticTap(btn, [6]);
  });
}
...
function initNavIndicator() {
  ...
  requestAnimationFrame(() => {
    setTimeout(() => updateNavIndicator('homeScreen'), 50);
  });
}
...
function _renderGreeting(st) {
  ...
  const total = (st.ok||0)+(st.wrongCount||0);
  const subs  = [
    'استمر في التقدم 💪',
...
function _bacDays() {
  // حساب صحيح: مقارنة الأيام فقط بدون وقت
  const now = new Date();
  const todayMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const bacMidnight   = new Date(2026, 5, 7); // 7 يونيو 2026
  const diff = bacMidnight - todayMidnight;
...
/* ── Screen history for directional transitions ── */
const _screenHistory = ['homeScreen'];

function showScreen(id) {
  const prev = document.querySelector('.screen.active');
  const prevId = prev ? prev.id : null;

  /* Determine direction: back if id is in history before current */
  let dir = 'forward';
  const histIdx = _screenHistory.lastIndexOf(id);
  if (histIdx >= 0 && histIdx < _screenHistory.length - 1) {
    dir = 'back';
    /* Trim history to this point */
    _screenHistory.length = histIdx + 1;
  } else if (id === 'homeScreen') {
    dir = 'back';
    _screenHistory.length = 1;
  } else {
...
function updateNavIndicator(id) {
  ...
  items.forEach(item => {
    item.classList.remove('active', 'bounce-anim');
  });

  // handle center button
  const centerBtn = document.querySelector('.nav-center-btn');
  ...
  if (active) {
    ...
    // SVG icon bounce animation
    const svg = active.querySelector('.nav-svg');
    if (svg) {
      svg.style.animation = 'none';
      void svg.offsetWidth;
      svg.style.animation = '';
      active.classList.add('bounce-anim');
      setTimeout(() => active.classList.remove('bounce-anim'), 500);
    }
...
function navClick(el, screenId, event) {
  _addNavRipple(el, event);
  showScreen(screenId);
}

function navCenterClick(el, screenId, event) {
  _addCenterRipple(el, event);
  // bounce center inner
  const inner = el.querySelector('.nav-center-inner');
  if (inner) {
    inner.style.transform = 'scale(0.85)';
    setTimeout(() => { inner.style.transform = ''; }, 160);
  }
  showScreen(screenId);
}

function _addNavRipple(el, event) {
  const wrap = el.querySelector('.nav-icon-wrap');
  ...
  rip.style.cssText =
    `position:absolute;border-radius:50%;pointer-events:none;
     background:rgba(139,92,246,0.28);width:40px;height:40px;
     left:${x - 20}px;top:${y - 20}px;
     animation:rippleOut 0.50s ease-out forwards;`;
  wrap.style.overflow = 'hidden';
  wrap.appendChild(rip);
  setTimeout(() => rip.remove(), 550);
}

function _addCenterRipple(el, event) {
  const inner = el.querySelector('.nav-center-inner');
  if (!inner || !event) return;
  ...
  rip.style.cssText =
    `position:absolute;border-radius:50%;pointer-events:none;
     background:rgba(255,255,255,0.22);width:60px;height:60px;
...
function _onShow(id) {
  switch(id) {
    ...
    case 'quizScreen':       if(typeof initQuiz==='function')          initQuiz();          break;
    case 'examScreen':       if(typeof initExam==='function')          initExam();          break;
    ...
    case 'pomodoroScreen':   if(typeof Pomodoro!=='undefined')         Pomodoro.init();     break;
    case 'studyPlanScreen':  if(typeof renderStudyPlan==='function')   renderStudyPlan();   break;
    case 'azkarScreen':      if(typeof initAzkar==='function')         initAzkar();         break;
  }
}
...
function closeOverlay() {
  ...
  if (sheet) sheet.classList.remove('open', 'show');
  const o = document.getElementById('overlay');
  if (o) o.classList.remove('show');
}
...
/* Legacy — kept for backward compatibility with HTML onclick="addRipple(event)" */
function addRipple(e) {
  _rippleOnElement(e.currentTarget, e.clientX, e.clientY, 'accent');
}

/* v2 — proportional ripple. Size = 2× the element's diagonal.
   Tinted white on dark surfaces, accent on light surfaces. */
function _rippleOnElement(el, x, y, tint) {
  if (!el) return;
  ...
  ripple.className = 'ripple-v2' + (tint === 'accent' ? ' accent' : '');
  ripple.style.cssText =
    `width:${size}px;height:${size}px;left:${relX}px;top:${relY}px;position:absolute;`;

  /* Ensure host has overflow:hidden */
  const prev = el.style.overflow;
  el.style.overflow = 'hidden';
  el.style.position = el.style.position || 'relative';
  el.appendChild(ripple);

  const dur = Math.min(600, Math.max(350, size * 0.8));
  setTimeout(() => {
    ripple.remove();
...
/* Haptic feedback — visual + device vibration */
function hapticTap(el, pattern) {
  if (!el) return;
  el.classList.add('haptic-press');
  setTimeout(() => el.classList.remove('haptic-press'), 200);
  if (navigator.vibrate) {
    navigator.vibrate(pattern || [8]);
  }
}

function hapticCorrect(el) {
  if (!el) return;
  el.classList.add('haptic-correct');
  setTimeout(() => el.classList.remove('haptic-correct'), 400);
  if (navigator.vibrate) navigator.vibrate([10, 40, 20]);
}

function hapticWrong(el) {
  if (!el) return;
  el.classList.add('haptic-wrong');
  setTimeout(() => el.classList.remove('haptic-wrong'), 450);
...
function _renderSpSummaryStats(st) {
  ...
  const totalQ     = (st.ok || 0) + (st.wrongCount || 0);
